...
class CanvasSegment(Line):
    def __init__(self, pointA, pointB, color='black', width=2, dash=None, arrow=None, method=Tools.M_VY):
    ...
    def show(self, field):
        ...
        if self.method == Tools.M_CDA:
            self.updateUseCDA(field)
        elif self.method == Tools.M_BREZENHAM_FLOAT:
            self.updateUseBrezReal(field)
        elif self.method == Tools.M_BREZENHAM_INT:
            self.updateUseBrezInt(field)
        elif self.method == Tools.M_BREZENHAM_ELIMINATION:
