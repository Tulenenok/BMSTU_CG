...
class CanvasPolLine:
    def __init__(self, points, color=Settings.COLOR_LINE, width=2, showComments=True, method=Tools.M_CDA):
        self.points = points
    ...
    def addPoint(self, field, newPoint):
        if len(self.points) > 0:
            self.lines.append(CanvasSegment(self.points[-1], newPoint, self.colorLine, method=self.method))
    ...
    def updateLines(self):
        ...
        for i in range(len(self.points) - 1):
            self.lines.append(CanvasSegment(self.points[i], self.points[i + 1], self.colorLine, method=self.method))
    ...
    def isEqualPol(self, pointStart, pointEnd):
        if len(self.points) < 2:
            return False
        return Point.isPointsEqual(pointStart, self.points[0]) and Point.isPointsEqual(pointEnd, self.points[1]) or \
               Point.isPointsEqual(pointEnd, self.points[0]) and Point.isPointsEqual(pointStart, self.points[1])
