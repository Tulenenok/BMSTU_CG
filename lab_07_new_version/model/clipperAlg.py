...
def cut(segmentPol, clipperPol):
    ...
    eps = 1e-6
    """ Поиск дальнего от p1 пересечения """
    ...
    spoint = findS(findT(p1, xLeft, xRight, yDown, yUp))
    if spoint != 0:
        return []

    """ теперь p1 -- это одна из точек пересечения отрезка с областью """
    """ второй точки может не быть, если одна из точек изначально лежала внутри области """

    if isP1inClipper:
        return [round(p1[0]), round(p1Copy[0])]

    if isP2inClipper:
        return [round(p1[0]), round(p2Copy[0])]

    firstIn = p1
    p1 = p1Copy
    p2 = p2Copy

    """ поиск второго пересечения """
    while abs(p1[0] - p2[0]) > eps and abs(p1[1] - p2[1]) > eps:
        mid = ((p2[0] + p1[0]) / 2, (p2[1] + p1[1]) / 2)
        tmid = findT(mid, xLeft, xRight, yDown, yUp)
        smid = findS(tmid)
    ...
    """ вернула то, что хочу закрасить """
    print(xLeft, xRight, yDown, yUp)
    print(round(firstIn[0]), round(p2[0]))
    return [round(firstIn[0]), round(p2[0])]
