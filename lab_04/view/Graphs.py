...
class Analysis:
    def __init__(self, root, canva):
        ...
        self.segmentForm = XYForm(self.z, Settings.COLOR_MAIN_BG, 'Center', Settings.WIDTH_INPUT,
                       lambda: self.plug(), '  Show  ', showButton=False)
        self.segmentForm.show(Settings.COLOR_MAIN_BG, 10, 10)
        self.segmentForm.insertXY(0, 0)
        ...
        self.addbtn = Button(self.z, text='add r', command=self.add)
        ...
        self.menu.add_cascade(label='Clean', menu=self.__makeDropDown({'Clean all': lambda: self.clearResult(),
                                                                       'Clean center': lambda: self.segmentForm.insertXY('', ''),
                                                                       'Clean radius': lambda: self.alphaFrame.clearAll(),
                                                                       'Clean graph': lambda: self.clear(),
                                                                       'Clean canva': lambda: self.canva.canva.clear(),
                                                                      }))
        self.menu.add_cascade(label='Generate', menu=self.__makeDropDown({'Gen r': lambda: self.genAlphaField()}))
    ...
    def genAlphaField(self):
        ...
        u.resizable(0, 0)
        f = XYXForm(u, Settings.COLOR_MAIN_BG, 'Gen radius', Settings.WIDTH_INPUT,
                       lambda: self.gen(u, f), '  Gen  ', labels=['N', 'Rb', 'Re'])
        f.insertXY(100, 0, 50)
        f.show(Settings.COLOR_MAIN_BG)

    def gen(self, u, f):
        n, rBegin, rEnd = f.getXY()
        if not Tools.isFloat(rBegin) or not Tools.isFloat(rEnd) or not Tools.isInt(n):
            showinfo('Error', 'Неверный ввод [rBegin float, rEnd float, N int]')
            return
        ...
        print(step)
        for i in range(n):
            self.alphaFrame.addString(str(rBegin))
            rBegin += step
            print(rBegin, rEnd, step)
    ...
    def changeColors(self):
        ...
        self.colors = [Settings.COLOR_CANONICAL, Settings.COLOR_PARAMETRIC, Settings.COLOR_B, Settings.COLOR_MIDDLE_POINT, Settings.COLOR_WU]
    ...
    def add(self):
        ...
        if not Tools.isInt(alpha):
            showinfo('Error', 'Неверно указан радиус')
            return
        if self.alphaFrame.isInYet(alpha):
            showinfo('Error', 'Радиус уже добавлен')
            return
    ...
    def go(self):
        ...
        for alpha in alphas:
            ...
            self.canva.canva.addCircle(start.x, start.y, end.x, end.y, self.colors[-1])
