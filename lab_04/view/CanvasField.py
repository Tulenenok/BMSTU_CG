...
from model.circles import FUNC_METHODS
from model.ellipses import FUNC_ELLIPS
...
class CartesianField(CoordGrid):
    ...
    def click(self, event):
        showinfo('Sorry', 'Лаба писалась в мега скоростном режиме, поэтому создание кружочков досутпно только через поля ввода, Извините')
...
class PolygonField(CartesianField):
    def __init__(self, rootFrame, root, **kwargs):
        ...
        self.method = Tools.M_CANONICAL

    def click(self, event):
        showinfo('Лапки',
                 'Лаба писалась в мега скоростном режиме, поэтому создание кружочков доступно только через поля ввода. '
                 'Прошу простить великодушно! (>_<)')
    ...
    def circle_info(self, root, XEvent, YEvent):
        for pol in self.polygons:
            if pol.center.isClick(self, XEvent, YEvent):
                try:
                    showinfo('Info', f'Center\nX = {pol.center.x}\nY = {pol.center.y}\n\nRadius = {pol.radius}')
                except:
                    showinfo('Info', f'Center\nX = {pol.center.x}\nY = {pol.center.y}\n\na = {pol.a}\nb = {pol.b}')
            else:
                for i, point in enumerate(pol.points):
    ...
    def startNewPolygon(self, event):
        self.polygons.append(CanvasPolCircle([], color=self.colorNowPol, method=self.method))
    ...
    def delCircle(self, center, r):
        wasDel = False
        for i, pol in enumerate(self.polygons):
            try:
                rc = pol.isEqualPol(center, r)
            except:
                continue
    ...
    def delEllipse(self, center, a, b):
        wasDel = False
        for i, pol in enumerate(self.polygons):
            try:
                rc = pol.isEqualPol(center, a, b)
            except:
                continue
    ...
    def addCircle(self, x, y, r, color=None):
        wasColor = self.colorNowPol
        if color:
            self.colorNowPol = color

        center = CanvasPoint(x, y, self.colorNowPol, showComments=self.ShowComments)
        self.polygons.append(CanvasPolCircle(center, r, self.colorNowPol, method=self.method))

        self.colorNowPol = wasColor

        return self.polygons[-1]

    def addEllipse(self, x, y, a, b, color=None):
        wasColor = self.colorNowPol
        if color:
            self.colorNowPol = color

        center = CanvasPoint(x, y, self.colorNowPol, showComments=self.ShowComments)
        self.polygons.append(CanvasPolEllipse(center, a, b, self.colorNowPol, method=self.method))
    ...
    def copy(self, window, XEvent, YEvent, method, color):
        for pol in self.polygons:
            ...
            if pol.center.isClick(self, XEvent, YEvent):
                ...
                self.addCircle(pol.center.x, pol.center.y, pol.radius)
...
class WrapCanva:
    ...
    def bind(self):
        ...
        submenu.add_command(label="Copy use canonic", command=lambda: self.canva.copy(self.window, self.Xevent, self.Yevent, Tools.M_CANONICAL, Settings.COLOR_CANONICAL))
        submenu.add_command(label="Copy use param", command=lambda: self.canva.copy(self.window, self.Xevent, self.Yevent, Tools.M_PARAMETRIC, Settings.COLOR_PARAMETRIC))
        submenu.add_command(label="Copy use Bresenham", command=lambda: self.canva.copy(self.window, self.Xevent, self.Yevent, Tools.M_BREZENHAM, Settings.COLOR_B))
        submenu.add_command(label="Copy use mid point", command=lambda: self.canva.copy(self.window, self.Xevent, self.Yevent, Tools.M_MIDDLE_POINT, Settings.COLOR_MIDDLE_POINT))
        submenu.add_command(label="Copy use lib", command=lambda: self.canva.copy(self.window, self.Xevent, self.Yevent, Tools.M_USUAL, Settings.COLOR_USUAL))
        ...
        self.pointMenu.add_command(label="Info",
                                   command=lambda: self.canva.circle_info(self.window, self.Xevent, self.Yevent))
        ...
        #self.mainMenu.add_command(label="Shift", command=lambda: self.action(CircleFrame))
    ...
    def sh(self, u, f, pol):
        ...
        for i in range(len(Tools.METHODS)):
            j += 1
            if Tools.METHODS[i] == pol.method or i < 4 and (FUNC_METHODS[i] == pol.method or FUNC_ELLIPS[i] == pol.method):
                j -= 1
                continue
            self.canva.method = Tools.METHODS[i]
            self.canva.colorNowPol = Settings.M_COLORS[i]

            try:
                self.canva.addCircle(pol.center.x + j * x, pol.center.y + j * y, pol.radius)
            except:
                self.canva.addEllipse(pol.center.x + j * x, pol.center.y + j * y, pol.a, pol.b)

        u.destroy()

        self.canva.method = wasMethod
        self.canva.colorNowPol = wasColor
        self.canva.update()
        self.canva.save()
