...
class PolygonField(CartesianField):
    ...
    def createPromptLine(self, event):
        if self.promptLine:
            self.delete(self.promptLine)
    ...
    def createPerpendicularPromptLine(self, event):
        ...
        startX = self.XShiftPC(self.polygons[-1].points[-1].x)
        startY = self.YShiftPC(self.polygons[-1].points[-1].y)
        endX = event.x
        endY = event.y
    ...
    def drawSegment(self):
        self.segmentOrClipper = True
        self.polygons[-1].segmentOrClipper = True
        self.polygons[-1].colorPoints = Settings.COLOR_LINE
        self.polygons[-1].colorLine = Settings.COLOR_LINE
        self.colorNowPol = Settings.COLOR_LINE

    def drawClipper(self):
        self.segmentOrClipper = False
        self.polygons[-1].segmentOrClipper = False
        self.polygons[-1].colorPoints = Settings.COLOR_NEW_CLIPPER
        self.polygons[-1].colorLine = Settings.COLOR_NEW_CLIPPER
        self.colorNowPol = Settings.COLOR_NEW_CLIPPER
    ...
    def myUpdate(self):
        ...
        for pol in self.polygons:
            if not pol.segmentOrClipper:          # вырезанный полигон
                pol.reShow(self)
                cutPixels += pol.pixels

        for pol in self.polygons:
            if pol.segmentOrClipper:              # отрисовка с вырезанными
