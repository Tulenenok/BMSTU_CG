...
class menuFrame:
    ...
    def __info_programm(self):
        showinfo('Info', 'Программа была создана 09.05 (но это не точно)\nАлгоритм Кируса Бека отсечения отрезка.\n\n'
                         'Краткая справка: чтобы замкнуть отсекатель пригодиться ctrl+space, чтобы быстро поменять цвет на рандомный ctrl+n')